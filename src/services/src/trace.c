...
/* System trace message queue event */
#define EVT_TRACE_MSG_QUEUE EVT_MASK(29)
...
{
    uint32_t TickCount = bos_SystemTick;
...
/*!
 * Function : Manage trace queue event
 * Context  : Task
 */
void SysTrace_ManageMsgQueueEvent(void)
{
    BOS_WaitEvent(EVT_TRACE_MSG_QUEUE);
    SysTrace_PrintOut();
}
